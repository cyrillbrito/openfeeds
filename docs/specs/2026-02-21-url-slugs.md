...
Migration: `pnpm -C packages/db drizzle-kit generate --name add-slugs`
...
```bash
pnpm add slug --filter packages/shared
pnpm add stopword --filter packages/shared
pnpm add -D @types/stopword --filter packages/shared
# @types/slug is also needed as a dev dependency
pnpm add -D @types/slug --filter packages/shared
```
