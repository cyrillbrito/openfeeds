...
| -------------------- | --------------------------------------------- | ---------------------- | ------------------------------------------------------------------ |
| `POSTHOG_PUBLIC_KEY` | `packages/domain`, `apps/server`, `apps/web`  | No                     | Disabled when absent. Web reads it at runtime via `/api/config`.   |
...
- **Identify** reactively when session loads (`posthog.identify(userId, { email, name, created_at })`)
- **Reset** on sign-out (`posthog.reset()`)

Locations: `apps/web/src/routes/_frame.tsx` (identify), `apps/web/src/components/UserMenu.tsx` (reset).
