...
- **Client:** `posthog.captureException()` in collection error handlers (`collectionErrorHandler`, `shapeErrorHandler`) and unexpected auth errors (sign-in, sign-up, reset-password, forgot-password). Client-side captures only see the post-serialization error message (no `.cause`). Note: sync/session errors from `collectionErrorHandler` and `shapeErrorHandler` appear as **exceptions** in PostHog (not named events), tagged with `context`, `errorName`, and `status` properties.
